...
############################################  CREATED BY MUHAMMAD AMMAR ABID  #################################
from tkinter import *
import random
import pyodbc
    ...
    def __init__(self,root):
        ...
        def add():
            con = pyodbc.connect((r'DRIVER={Microsoft Access Driver (*.mdb, *.accdb)};'
                                  r'DBQ=E:\\Ammar work\\My Python\\Invoice system\\Database.accdb;'))
            cursor1 = con.cursor()
            cursor1.execute(
                f"INSERT INTO infor([NameOfCus], [Phone], [Billno], [BathSoap], [FaceCream], [FaceWash], [HairSpray], [BodyLotion])"
                f" values('{self.cus_name.get()}','{self.c_phone.get()}','{self.c_bill_no.get()}', '{self.bath_soap.get()}','{self.face_cream.get()}','{self.face_wash.get()}','{self.hair_spray.get()}','{self.body_lotion.get()}')")
            con.commit()
            messagebox.showinfo("Alert","One record has been added")
            con.close()
        ...
        #====================Bill Search Button===============#
        bill_btn = Button(F1,text = "Enter",bd = 7,relief = GROOVE,font = ("times new roman",12,"bold"),bg = bg_color,fg = fg_color, command = add)
